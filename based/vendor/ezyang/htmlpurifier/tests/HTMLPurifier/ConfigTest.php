...
class HTMLPurifier_ConfigTest extends HTMLPurifier_Harness
{
    ...
    public function testNormal()
    {
        ...
        $this->schema->add(
            'Element.IsotopeNames',
            array(1 => 'protium', 2 => 'deuterium', 3 => 'tritium'),
            'hash',
            false
        );
        ...
        $this->assertIdentical(
            $config->get('Element.IsotopeNames'),
            array(1 => 'protium', 2 => 'deuterium', 3 => 'tritium')
        );
        ...
        $this->assertIdentical(
            $config->get('Element.IsotopeNames'),
            array(238 => 'Plutonium-238', 239 => 'Plutonium-239')
        );
    ...
    public function testEnumerated()
    {
        ...
        $this->expectError(
            'Value not supported, valid values are: Yamaha, Conn-Selmer, Vandoren, Laubin, Buffet, other'
        );
    ...
    public function test_getHTMLDefinition_rawAfterSetupError()
    {
        $this->expectException(
            new HTMLPurifier_Exception("Cannot retrieve raw definition after it has already been setup")
        );
        $config = HTMLPurifier_Config::createDefault();
        $config->chatty = false;
        $config->getHTMLDefinition();
    ...
    public function test_getHTMLDefinition_inconsistentOptimizedError()
    {
        $this->expectError("Useless DefinitionID declaration");
        $this->expectException(
            new HTMLPurifier_Exception("Inconsistent use of optimized and unoptimized raw definition retrievals")
        );
        $config = HTMLPurifier_Config::create(
            array('HTML.DefinitionID' => 'HTMLPurifier_ConfigTest->test_getHTMLDefinition_inconsistentOptimizedError')
        );
        $config->chatty = false;
    ...
    public function test_getHTMLDefinition_inconsistentOptimizedError2()
    {
        ...
        $config = HTMLPurifier_Config::create(
            array('HTML.DefinitionID' => 'HTMLPurifier_ConfigTest->test_getHTMLDefinition_inconsistentOptimizedError2')
        );
        $config->chatty = false;
        $config->getHTMLDefinition(true, true);
        $config->getHTMLDefinition(true, false);
    ...
    public function test_getHTMLDefinition_rawError()
    {
        $config = HTMLPurifier_Config::createDefault();
        $this->expectException(
            new HTMLPurifier_Exception('Cannot retrieve raw version without specifying %HTML.DefinitionID')
        );
