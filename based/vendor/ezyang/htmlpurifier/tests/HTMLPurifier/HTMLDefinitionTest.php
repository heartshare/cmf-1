...
class HTMLPurifier_HTMLDefinitionTest extends HTMLPurifier_Harness
{
    ...
    public function test_parseTinyMCEAllowedList()
    {
        ...
        $this->assertEqual(
            ...
            array(
                array('span' => true, 'strong' => true, 'a' => true),
    ...
    public function test_AllowedAttributes_local_invalidAttributeDueToMissingElement()
    {
        ...
        $this->expectError(
            new PatternExpectation("/Cannot allow attribute 'style' if element 'foo' is not allowed\/supported/")
        );
        $this->assertPurification_AllowedAttributes_local_p_style();
    }
    ...
    public function test_ForbiddenAttributes_incorrectSyntax()
    {
        $this->config->set('HTML.ForbiddenAttributes', 'b.style');
        $this->expectError(
            "Error with b.style: tag.attr syntax not supported for HTML.ForbiddenAttributes; use tag@attr instead"
        );
    ...
    public function test_ForbiddenAttributes_incorrectGlobalSyntax()
    {
        ...
        $this->expectError(
            "Error with *.style: *.attr syntax not supported for HTML.ForbiddenAttributes; use attr instead"
        );
        $this->assertPurification('<b style="float:left;">Test</b>');
    }
    ...
    {
        $this->expectError(
            "Required attribute 'src' in element 'img' was not allowed, which means 'img' will not be allowed either"
        );
