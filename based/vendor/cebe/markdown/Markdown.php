...
class Markdown extends Parser
{
    ...
    {
        while (isset($lines[$current]) && preg_match(
                '/^ {0,3}\[(.+?)\]:\s*(.+?)(?:\s+[\(\'"](.+?)[\)\'"])?\s*$/',
                $lines[$current],
                $matches
            )) {
            ...
            if (isset($matches[3])) {
                ...
                if (isset($lines[$current + 1]) && preg_match(
                        '/^\s+[\(\'"](.+?)[\)\'"]\s*$/',
                        $lines[$current + 1],
                        $matches
                    )
    ...
    protected function renderCode($block)
    {
        ...
        return "<pre><code$class>" . htmlspecialchars(
            implode("\n", $block['content']) . "\n",
            ENT_NOQUOTES,
            'UTF-8'
        ) . '</code></pre>';
    ...
    protected function renderList($block)
    {
        ...
        foreach ($block['items'] as $item => $itemLines) {
            ...
            if (!isset($block['lazyItems'][$item])) {
                $firstPar = [];
                while (!empty($itemLines) && rtrim($itemLines[0]) !== '' && $this->identifyLine(
                        $itemLines,
                        0
                    ) === 'paragraph') {
    ...
    {
        if (strpos($markdown, ']') !== false && preg_match(
                '/\[((?:[^][]|(?R))*)\]/',
                $markdown,
                $textMatches
            )
